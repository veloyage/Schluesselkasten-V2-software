...
class BQ25628:
    # Device I2C address (keep existing)
    DEFAULT_ADDR = 0x6A
    ...
    def enable_charging(self, enable=True):
        reg = self._read_register(self.REG_CHARGER_CONTROL_0)
        if enable:
            reg |= 1 << 5
        else:
            reg &= ~(1 << 5)
    ...
    def enable_tempsense(self, enable=True):
        reg = self._read_register(self.REG_NTC_CONTROL_0)
        if enable:
            reg &= ~(1 << 7)
        else:
            reg |= 1 << 7
    ...
    # ADC functions
    def enable_adc(self, enable=True):
        reg = self._read_register(self.REG_ADC_CONTROL)
        if enable:
            reg |= 1 << 7
        else:
            reg &= ~(1 << 7)
    ...
    def read_adc_values(self):
        ...
        return {
            'ibus': int.from_bytes(self._read_register(self.REG_IBUS_ADC, 2), 'little', signed=True),
            'ibat': int.from_bytes(self._read_register(self.REG_IBAT_ADC, 2), 'little', signed=True),
            'vbus': int.from_bytes(self._read_register(self.REG_VBUS_ADC, 2), 'little', signed=False),
            'vpmid': int.from_bytes(self._read_register(self.REG_VPMID_ADC, 2), 'little', signed=False),
            'vbat': int.from_bytes(self._read_register(self.REG_VBAT_ADC, 2), 'little', signed=False),
            'vsys': int.from_bytes(self._read_register(self.REG_VSYS_ADC, 2), 'little', signed=False),
            'ts': int.from_bytes(self._read_register(self.REG_TS_ADC, 2), 'little', signed=False),
            'tdie': int.from_bytes(self._read_register(self.REG_TDIE_ADC, 2), 'little', signed=True)
        }
    ...
    def get_part_id(self):
        ...
        """
        return self._read_register(self.REG_PART_INFORMATION) << 3

    def get_part_rev(self):
        """Read part information (REG0x38)
        Should read 2
        """
        return self._read_register(self.REG_PART_INFORMATION) & 0x07
